...
void displayIntervention() {
    ...
    getline(cin, name);
    cout << "Enter the number of ways you want to add to prevent it spreading: ";
    cin >> choice;

    while (choice--) {
        cout << "Enter the start day of the intervention: ";
        cin >> start;
        ...
        if (type == "q") type = "quarantine";
        else if (type == "v") type = "vaccination";
        else if (type == "d") type = "lockdown";
        else {
            cout << "Invalid Input" << endl;
            continue;
        }

        model.addIntervention(name, start, eff, type,
                              [](double current_DAYS, double infected, double beta) {
                                  return current_DAYS == 5;
                              },
                              [](double current_DAYS, double infected, double beta) {
                                  return infected > 100;
                              },
